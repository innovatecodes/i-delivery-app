namespace IDelivery.Infrastructure.Messaging.Templates;

internal static class EmailTemplate
{
    public static string Generate(string title, string message, string linkUrl, string linkText, string footerMessage)
    {
        return $@"
            <!DOCTYPE html>
            <html>
            <head>
                <meta charset='utf-8'>
                <style>
                    body {{ font-family: Arial, sans-serif; line-height: 1.6; color: #333; }}
                    .container {{ max-width: 600px; margin: 0 auto; padding: 20px; }}
                    .button {{ display: inline-block; padding: 12px 24px; background-color: #2563eb; color: white; text-decoration: none; border-radius: 6px; }}
                    .footer {{ margin-top: 20px; font-size: 12px; color: #666; }}
                </style>
            </head>
            <body>
                <div class='container'>
                    <h2>{title}</h2>
                    <p>{message}</p>
                    <p style='text-align: center; margin: 30px 0;'>
                        <a href='{linkUrl}' class='button'>{linkText}</a>
                    </p>
                    <p>Ou copie e cole este link no navegador:</p>
                    <p style='word-break: break-all; color: #2563eb;'>{linkUrl}</p>
                    <p>Este link expira em 24 horas.</p>
                    <div class='footer'>
                        <p>{footerMessage}</p>
                        <p>&copy; {DateTime.UtcNow.Year} iDelivery. Todos os direitos reservados.</p>
                    </div>
                </div>
            </body>
            </html>";
    }
}





